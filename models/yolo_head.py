...
class YOLOXHead(nn.Layer):
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_classes = labels[batch_idx, 0:num_gt, 0]
                ...
                if pred_ious_this_matching.shape[0]!=0:
                    cls_target *= pred_ious_this_matching.unsqueeze(-1)
                else:
                    cls_target *= 0
                obj_target = fg_mask.unsqueeze(-1)
                # if matched_gt_inds[0].numpy()!=10000:
                    # reg_target = gt_bboxes_per_image[matched_gt_inds]
                reg_target = paddle.to_tensor(gt_bboxes_per_image.numpy()[matched_gt_inds.numpy()])
                # else:
                #     reg_target = paddle.zeros([0,gt_bboxes_per_image.shape[1]])
                if self.use_l1:
                ...
                # print("matched_gt_inds[0]:",matched_gt_inds[0])
        ...
        if num_gt != 0 and matched_gt_inds[0].numpy()!=10000:
        ...
        loss_obj = (
            self.bcewithlog_loss(obj_preds.reshape([-1, 1]), obj_targets.cast(dtype))
        ).sum() / num_fg
        loss_cls = (
    ...
    def get_assignments(
        ...
        cost = (
            ...
            + 100000.0 * (paddle.to_tensor(~(is_in_boxes_and_center.numpy()), paddle.float32))
        )
    ...
    def get_in_boxes_info(
        ...
        is_in_boxes_anchor = numpy.logical_or(is_in_boxes_all.numpy(),is_in_centers_all.numpy())
        is_in_boxes_anchor = paddle.to_tensor(is_in_boxes_anchor )
        idx = is_in_boxes_anchor.nonzero()
        is_in_boxes_anchor_ = paddle.expand_as(is_in_boxes_anchor,is_in_boxes)
        if idx.shape[0]!=0:
            is_in_boxes_and_center = (
                numpy.logical_and(
                is_in_boxes.astype("int").gather(idx, axis=1).astype("bool").numpy(),
                is_in_centers.astype("int").gather(idx, axis=1).astype("bool").numpy()
                )
            )
            is_in_boxes_and_center = paddle.to_tensor(is_in_boxes_and_center )
        else:
            is_in_boxes_and_center = paddle.to_tensor(0)
        return is_in_boxes_anchor, is_in_boxes_and_center

    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        # Dynamic K
        # ---------------------------------------------------------------
        matching_matrix = paddle.zeros_like(cost) # num_gt,taget_gt 
        ...
        dynamic_ks = paddle.clip(topk_ious.sum(1).cast(paddle.int64), min=1)
        ...
        if paddle.sum(paddle.to_tensor(anchor_matching_gt.numpy() > 1).astype("int32")).numpy() > 0:
            anchor_matching_gt = anchor_matching_gt.numpy()
            cost_argmin = numpy.argmin(cost.numpy()[:, anchor_matching_gt > 1], axis=0)
        ...
        matched_gt_inds = matching_matrix[:, fg_mask_inboxes]
        matched_gt_inds = paddle.to_tensor(matched_gt_inds).astype(int).numpy()
...
################################################################################################################
        ...
        # temp_indicies =  fg_mask.clone().nonzero().numpy()
        # if temp_indicies.shape[0]!=0:
        #     fg_mask[temp_indicies[:, 0]] = fg_mask_inboxes
        ...
        # else:
        #     gt_matched_classes = paddle.to_tensor(0)
        #     matched_gt_inds = paddle.to_tensor(10000)
#################################################################################################################
        matched_gt_inds = paddle.to_tensor(matched_gt_inds)
        fg_mask_inboxes = paddle.to_tensor(fg_mask_inboxes).argmax(0)
        gt_matched_classes = paddle.to_tensor(gt_matched_classes)
        matching_matrix = paddle.to_tensor(matching_matrix)
        num_fg = paddle.to_tensor(num_fg)

        pred_ious_this_matching = (matching_matrix * pair_wise_ious).sum(0)[(fg_mask_inboxes).astype(int)]
