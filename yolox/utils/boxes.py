...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        conf_mask = (image_pred[:, 4] * class_conf.squeeze() >= 0.005).squeeze()
        class_pred = paddle.cast(class_pred, 'float32')
        detections = paddle.concat((image_pred[:, :5], class_conf, class_pred), 1).numpy()
        idx = paddle.masked_select(paddle.arange(0, detections.shape[0]), conf_mask).astype("int").numpy()
...
def bboxes_iou(bboxes_a, bboxes_b, xyxy=True):
    ...
    if bboxes_b.shape[0] !=0: 
        f = open("less_than_tl.txt", "w")
        f.write(str(tl))
        f = open("less_than_br.txt", "w")
        f.write(str(br))
        f.close()
        # en = paddle.less_than(tl,br)
        en = tl.numpy()<br.numpy()
        en = paddle.to_tensor(en)
        tl = paddle.to_tensor(tl)
        br = paddle.to_tensor(br)

        en = en.cast(tl.dtype).prod(axis=2)
        area_i = paddle.prod(br - tl, 2) * en  # * ((tl < br).all())
        out = area_i / (area_a.unsqueeze(-1) + area_b - area_i)
    else:
        out = paddle.randn([bboxes_a.shape[0],0 ])
    return out
