...
class YOLOXHead(nn.Layer):
    ...
    def get_assignments(
        ...
        gt_cls_per_image = F.one_hot(paddle.cast(gt_classes, paddle.int64), self.num_classes).astype("float32").unsqueeze(1)
        if  num_in_boxes_anchor!=0:
            gt_cls_per_image = paddle.expand(gt_cls_per_image, [1*gt_cls_per_image.shape[0], num_in_boxes_anchor*gt_cls_per_image.shape[1], 1*gt_cls_per_image.shape[2]])
        else:
            gt_cls_per_image = paddle.randn([1*gt_cls_per_image.shape[0],0, 1*gt_cls_per_image.shape[2]])
            # gt_cls_per_image = paddle.expand(gt_cls_per_image, [1*gt_cls_per_image.shape[0], num_in_boxes_anchor*gt_cls_per_image.shape[1], 1*gt_cls_per_image.shape[2]])
        ...
        with paddle.amp.auto_cast(enable=False):
            if cls_preds_.shape[0]!=0:
                ...
                cls_preds_ = paddle.randn([num_gt,0,cls_preds_.shape[1]])
            if cls_preds_.shape[1]!=0:
                pair_wise_cls_loss = paddle.sum(
                    F.binary_cross_entropy(cls_preds_.sqrt_(), gt_cls_per_image, reduction="none"),
                    axis=-1)
            else:
                pair_wise_cls_loss = paddle.randn([num_gt,0])


        del cls_preds_
        cost = (pair_wise_cls_loss + 3.0 * pair_wise_ious_loss  + 100000.0 * (paddle.logical_not(is_in_boxes_and_center)) )
    ...
    def get_in_boxes_info(
        ...
        is_in_boxes =  is_in_boxes.numpy()
        is_in_centers = is_in_centers.numpy()
        is_in_boxes_and_center = (
            is_in_boxes[:, is_in_boxes_anchor] & is_in_centers[:, is_in_boxes_anchor]
        )
        return paddle.to_tensor(is_in_boxes_anchor),paddle.to_tensor(is_in_boxes_and_center)
