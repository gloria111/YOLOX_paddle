...
#####################continue####################################################
    ...
    #         reshape_b = paddle.reshape(b, (n_anchors, -1))
        
    #         # b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
    #         b = paddle.ones_like(reshape_b)
    #         b = b*(-math.log((1 - prior_prob) / prior_prob))

    #         # conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
    #         conv.bias = paddle.create_parameter(
    #                     shape=(paddle.reshape(b, (-1,))).shape,
    ...
    #         bias = conv.bias
    #         b = paddle.reshape(bias,(self.n_anchors, -1))
    #         b = paddle.ones_like(b)
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        expanded_strides = []
        #self.num_classes=80
        #xin(3:[64,128,80,80];[64, 256, 40, 40],[64, 512, 20, 20])
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            #self.cls_preds[k].bias=[1,80],[1,80]
            ...
            if self.training:
                ...
                output, grid = self.get_output_and_grid(
                    output, k, stride_this_level,'float32'
                )
                x_shifts.append(grid[:, :, 0])
                y_shifts.append(grid[:, :, 1])
                expanded_strides.append(
                    (paddle.ones((1, grid.shape[1]))
                   *(stride_this_level))
                    .astype('float32')
                )
                if self.use_l1:
                    ...
                    reg_output = paddle.reshape(reg_output,(batch_size, self.n_anchors, 4, hsize, wsize))
                    reg_output = paddle.transpose(reg_output,(0, 1, 3, 4, 2))
                    reg_output = paddle.reshape(reg_output,( batch_size, -1, 4))
        ...
        if self.training:
            ...
            if self.decode_in_inference:
                return self.decode_outputs(outputs, dtype='float32')
            else:
                return outputs
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        if grid.shape[2:4] != output.shape[2:4]:
            ...
            grid = paddle.reshape(grid,(1, 1, hsize, wsize, 2)).astype(dtype)
            self.grids[k] = grid
        ...
        output[..., 2:4] = numpy.exp(output[..., 2:4]) * stride
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        grids = paddle.cast(grids,dtype)
        strides = paddle.concat(strides, axis=1)
        strides = paddle.cast(strides,dtype)

        outputs = outputs.numpy()
        grids = grids.numpy()
        strides = strides.numpy()

        outputs[..., :2] = (outputs[..., :2] + grids) * strides
        # outputs[..., 2:4] = torch.exp(outputs[..., 2:4]) * strides
        outputs[..., 2:4] = numpy.exp(outputs[..., 2:4]) * strides
        
        grids = paddle.to_tensor(grids)
        strides = paddle.to_tensor(strides)
    ...
    def get_losses(
        ...
        obj_preds = outputs[:, :, 4] # [batch, n_anchors_all, 1]
        cls_preds = outputs[:, :, 5:]  # [batch, n_anchors_all, n_cls]
        bbox_preds = paddle.to_tensor(bbox_preds)
        obj_preds = paddle.to_tensor(obj_preds).unsqueeze(-1) 
        cls_preds = paddle.to_tensor(cls_preds)
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                cls_target = F.one_hot(
                    gt_matched_classes.to(paddle.int64), self.num_classes
                ) 
        ...
        num_fg = max(num_fg, 1)
        bbox_preds = paddle.reshape(bbox_preds,(-1, 4))
        obj_preds = paddle.reshape(obj_preds,(-1, 1))
        loss_iou = (
            self.iou_loss(bbox_preds[fg_masks], reg_targets)
        ).sum() / num_fg
        # loss_obj = (
        ...
        cls_preds = paddle.reshape(cls_preds, [-1,self.num_classes])
        loss_cls = (
            self.bcewithlog_loss(
                cls_preds[fg_masks], cls_targets
            )
        ).sum() / num_fg
        origin_preds = paddle.reshape(origin_preds,(-1, 4))
        if self.use_l1:
            loss_l1 = (
    ...
    def get_assignments(
        ...
        gt_cls_per_image = (
            F.one_hot(gt_classes.to(paddle.int64), self.num_classes)
            .float()
            .unsqueeze(1)
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        topk_ious, _ = paddle.topk(ious_in_boxes_matrix, n_candidate_k, dim=1)
        dynamic_ks = paddle.clamp(topk_ious.sum(1).int(), min=1)
        ...
        if (anchor_matching_gt > 1).sum() > 0:
            _, cost_argmin = paddle.min(cost[:, anchor_matching_gt > 1], axis=0)
