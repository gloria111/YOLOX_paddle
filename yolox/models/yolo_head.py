...
#####################continue####################################################
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            ...
            outputs = paddle.concat(
                [x.flatten(start_dim=2) for x in outputs], axis=2
            ).permute(0, 2, 1)
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        for (hsize, wsize), stride in zip(self.hw, self.strides):
            yv, xv = paddle.meshgrid([paddle.arange(hsize), paddle.arange(wsize)])
            grid = paddle.stack((xv, yv), 2).view(1, -1, 2)
            grids.append(grid)
            shape = grid.shape[:2]
        ...
        grids = paddle.concat(grids, dim=1).type(dtype)
        strides = paddle.concat(strides, dim=1).type(dtype)
