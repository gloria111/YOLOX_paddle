...
import sys
sys.path.append(r'/home/aistudio/YOLOX/')
# import torch
import paddle
...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.device = "gpu:{}".format(self.local_rank)
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            loss_str = ", ".join(
                ...
                ["{}: {:.1f}".format(k, float(v.latest)) for k, v in loss_meter.items()]
        
            )
            # loss_str = ", "
            # for k, v in loss_meter.items():
            #     print("v.latest:",v.latest)
            #     loss_str = loss_str.join( ["{}: {:.1f} , ".format(k, float(v.latest)) ])
